  ...
  try {
    ...
    if (type === 'deposit') {
      if (job.payment_status && job.payment_status !== 'pending_deposit') {
        return { statusCode: 400, body: JSON.stringify({ error: 'Deposit already handled for this job' }) };
      }
      ...
      if (remainderDollars <= 0) {
        return { statusCode: 400, body: JSON.stringify({ error: 'No balance due on this job' }) };
      }
      amountCents = remainderDollars * 100;
      description = `Balance due — ${job.service} (${job.vehicle})`;
    } else {
      return { statusCode: 400, body: JSON.stringify({ error: 'type must be "deposit" or "remainder"' }) };
    }
